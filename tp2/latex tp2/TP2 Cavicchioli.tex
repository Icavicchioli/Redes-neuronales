...
Claramente limitar a 40 mjestras hace que le cueste mucho generalizar, en especial si las muestras no son representativas del dataset. Basicamente con tan poco hace lo que quiere.

\subsection{Análisis global}

Dentro de los puntos importantes de esta consigna se destaca que el \textit{minibatch} (indirectamente el gradiente estocástico) aumenta la velocidad del proceso de aprendizaje, y se llegan a resultados igual de adecuados que si se usaran tamaños de lotes mayores. Esto es indispensable para la optimización en datasets grandes. Otro beneficio del uso de \textit{\textit{minibatch}}es es que cada ejecución es única (por el orden de las muestras), por lo que el camino que recorre en la minimización varía con la ejecución, lo que permite evitar mínimos locales que posiblemente trabarían un aprendizaje por gradiente determinístico.
