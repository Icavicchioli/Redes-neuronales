...
\section{Estado del arte en identificación no lineal}

Los modelos ARX (AutoRegressive with eXogenous input) describen sistemas dinámicos lineales como combinaciones de salidas e inputs retrasados.\cite{LjungNonlinearSI} Sus extensiones no lineales, los modelos NARX y NARMAX, reemplazan esa combinación lineal por una función no lineal de los mismos regresores e incorporan, en el caso NARMAX, términos explícitos de ruido.\cite{LjungNonlinearSI}

En este contexto, es frecuente implementar la función no lineal de un modelo NARX mediante redes neuronales, en particular perceptrones multicapa (MLP) y redes recurrentes, que permiten aproximar directamente la dinámica del sistema a partir de datos.\cite{Narendra1990} En esta monografía se utiliza precisamente un MLP autoregresivo de este tipo, lo que habilita una comparación directa con los modelos lineales analíticos e identificados del sistema de tanques.\cite{Narendra1990}
...
En esta sección se evita la mención de cotas como márgenes de fase, ganancia y estabilidad, debido a que no son relevantes para la monografía y, como indica la p.~581, sección 19.11 del libro \textit{Control System Design} \cite{Goodwin2000}, muchas nociones comunes para sistemas lineales no aplican para los no lineales.
...
% o
\renewcommand{\bibname}{Referencias} % para report/book
\bibliographystyle{ieeetr}  % o plain, alpha, etc.
\bibliography{biblio}
