...
Teniendo un controlador funcional, se procedió a generar muestras de este y clonarlo por medio de una red neuronal. El procedimiento es idéntico al empleado en identificación de sistemas, pero lo novedoso está en estudiar si una red neuronal puede reemplazar al controlador. Se espera que la red pueda emular al controlador pero sin las seguridades que trae el uso de un PID, tales como seguridad que va a operar en cierto rango de acciones de control y no va a caer en ciclos repetitivos, divergir de forma espuria o asegurar error nulo en estado estacionario.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=1\linewidth]{imgs/step_NN_PID2}
...
Ahora bien, lo interesante es lo siguiente. La imagen \ref{fig:cloncontrolpulsosref} muestra las salidas a lazo cerrado cuando varía la referencia. Se empeizan a observar problemas con el controlador clonado, principalmente que su error en estado estacionario no es nulo, como el del controlador original (el PI/PID). Es más, la figura \ref{fig:cloncontrolpulsosruido}, simulada con el esquema \ref{fig:esquemaruidoentradaplanta}, indica que hay algo fundamentalmente mal, ya que el controlador clonado no comprende que el ruido introducido en el lazo se debe eliminar y, en cambio, está respondiendo a estos como si fueran cambios en la referencia.
...
En retrospectiva, el \textit{dataset} nunca debería haber incluido la referencia, ya que es invariante con las perturbaciones en el lazo. Para ser justos, se entrenó otra red usando un set de datos que usa solo el error del lazo y salida. Dado los resultados previos, no se tienen demasiada fe en que funcione.
