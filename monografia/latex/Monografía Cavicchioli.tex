...
El modelado de este sistema hace uso de varias leyes físicas de la hidráulica. Iniciando, primero se plantea que el líquido es incompresible, por lo que el volumen solo varía si los caudales de entrada y salida no son iguales.
...
Las expresiones \eqref{ss1} y \eqref{ss2} se van a linealizar en torno al punto operativo del tanque, definido por:
g   = 9.81;
A1  = 0.015;
A2  = 0.02;
k12 = 5e-4;
...
$$
\overset{\,\circ }{X} = f(x_e,u_e) + \frac{df}{d x}|_{(x_e,u_e)} (x-x_e,u-u_e) +  \frac{df}{d u}|_{(x_e,u_e)} (x-x_e,u-u_e)
$$
...
Pero acá son las linealizadas, así que son sin el punto de trabajo. la entrada que tenga deberá ser - 0.002.

Y por un proceso similar se llega a que
