...
El segundo enfoque ideado surge del anterior. Si se puede lograr un buen desempeño con un modelo lineal, debería ser posible entrenar una red neuronal  que estime las salidas no lineales a partir de las lineales.

La figura \ref{fig:red_autoreg2} muestra el sistema armado. Se mantiene el mismo vector de entrada del experimento previo, con la diferencia que ahora las salida previas vienen del sistema lineal y no se predice la siguiente salida, sino que corrige la actual.
...
Inicialmente, se espera que este sistema logre mejor precisión que si solo se usara el lineal, pero no se puede saber si va a ser mejor que las otras redes. Además, se espera que sea más predecible, dado que una gran parte de la dinámica es fundamentalmente lineal.

\subsubsection{Entrenamiento}

Graf de performance de la red!

Se decidió entrenar 3 MLPs de 2, 5, 10 y 20 neuronas en la capa oculta. Se desea observar que complejidad de red se requiere para capturar el mapeo de la salida lineal a la no lineal.
